...
RUN dnf upgrade -y && \
    ...
    wget -q https://golang.org/dl/go1.16.5.linux-amd64.tar.gz -P /root/ && tar -C /usr/local -xzf /root/go1.16.5.linux-amd64.tar.gz && \
    rm -rf /root/go1.16.5.linux-amd64.tar.gz && \
    pip3 install -U pip && \
    pip install ansible && \
