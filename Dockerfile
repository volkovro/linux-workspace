...
RUN apt-get update && \
    ...
    wget -s https://github.com/mikefarah/yq/releases/download/v4.1.0/yq_linux_amd64 -O /usr/bin/yq && chmod +x /usr/bin/yq && \
    wget -q https://golang.org/dl/go1.15.7.linux-amd64.tar.gz -P /root/ && tar -C /usr/local -xzf /root/go1.15.7.linux-amd64.tar.gz && export PATH=$PATH:/usr/local/go/bin && \
